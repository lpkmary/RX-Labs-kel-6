...
if test == "Home👩🏻‍🔬":
    st.title(':green[Penyetaraan reaksi antara asam kuat dengan basa kuat]')
    st.markdown('''Hai users, selamat datang di web kami.😊''')
    st.markdown('''Aplikasi ini dapat digunakan untuk menyetarakan reaksi kimia dari suatu senyawa asam kuat dan basa kuat.''')

    tombol=st.button('OLEH KELOMPOK 6')
    if tombol:
        st.write('Kayla Alifa (2360154)')
        st.write('Mary Rianti Supriyanto (2360170)')
        st.write('Muhamad Renaldi Irawan (2360179)')
    ...
    def cariHasilWithKurung(asamKuat, basaKuat):
        ...
        if asamKuat != "H2SO4":
            ...
            basa = basaKuat.split("(")[0]
            asam = asamKuat.split("2")[1]
            left_side = f"{basa}({asam})"
            hasil = left_side + right_side
            temporary = f"{asamKuat} + {basaKuat} → {hasil}"
...
if test == "About Us":
    ...
    st.write(
    """
    Di Web Aplikasi Penyetaraan reaksi ini, kita menyediakan bagi kamu yang:
